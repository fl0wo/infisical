...
const migrationName = prompt("Enter the file name of the migration to remove: ");
...
execSync(
  // `npx knex migrate:make --knexfile ${path.join(__dirname, "../src/db/knexfile.ts")} -x ts ${formattedMigrationName}`,
    `npx knex migrate:down --knexfile ${path.join(__dirname, "../src/db/knexfile.ts")} ${formattedMigrationName}`,
  { stdio: "inherit" }
);
